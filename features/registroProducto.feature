...
Feature: Funcionalidad de registro de producto
    ...
    Scenario: Usuario registrado realiza el registro de un nuevo producto sin descripcion
        When el usuario selecciona la lista desplegable de Entidades
        And  el usuario va a la página de Artículos
        And  el usuario selecciona crear un nuevo articulo
        And  el usuario crea el formulario con Código "IP-16-2025-v16", Descripción "", Stock Actual "10", Costo "1000000", Precio venta "1200000" y selecciona la unidad de medida "Unidad" 
        Then el usuario se mantiene en la página de crear un nuevo articulo